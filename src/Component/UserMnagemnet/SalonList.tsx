// src/pages/UserManagement/SalonList.tsx
...
const SalonList: React.FC = () => {
    const [salons, setSalons] = useState<User[]>([]);
                                ...
                                {salons.map((salon) => (
                                    <tr key={salon._id} className="border-b border-gray-200 hover:bg-gray-50">
                                        ...
                                        <td className="py-3 px-4">
                                            <span className={`px-2 py-1 rounded-full text-xs ${salon.isSuspended ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                                                {salon.isSuspended ? 'Suspended' : 'Active'}
                                            </span>
                                        </td>
                                        <td className="py-3 px-4 space-x-2">
                                            ...
                                            <button
                                                onClick={() => handleStatusChange(salon._id, !salon.isSuspended)}
                                                className={`${salon.isSuspended ? 'text-green-600 hover:text-green-800' : 'text-yellow-600 hover:text-yellow-800'}`}
                                            >
                                                {salon.isSuspended ? 'Activate' : 'Suspend'}
                                            </button>
                                        </td>
                                    </tr>
                                ))}
